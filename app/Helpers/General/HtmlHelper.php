...
class HtmlHelper
{
     ...
     * @return HtmlString
     */
    public function formSubmit($title, $classes = 'btn btn-success pull-right') // btn-sm
    {
     ...
     * @return HtmlString
     */
    public function buttonOk($title, $target, $classes = 'btn btn-success pull-right', $attributes = [], $secure = null)
    {
        $defaults = ['class' => $classes];

        $attributes = $attributes + $defaults;

        $attributes['href'] = $this->url->asset($target, $secure);

        return $this->toHtmlString('<a'.$this->attributes($attributes).'>'.$title.'</a>'.PHP_EOL);
    }
}
